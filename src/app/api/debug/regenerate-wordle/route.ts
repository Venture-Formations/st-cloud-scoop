...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const entry of wordleData) {
      ...
      try {
        // Generate new definition using the database prompt (same as test-ai-prompts GET)
        const newDefinition = await AI_PROMPTS.wordleDefinition(word)

        // Generate new interesting fact using the database prompt (same as test-ai-prompts GET)
